      ...
      <v-card class="elevation-6">
        ...
        >
          <v-tab
            ...
            :href="'#year' + tab._id"
            class="tab-heading"
            @click="SetScoreGroup(tab._id, index)"
          >
            {{ tab._id }}
          </v-tab>
        </v-tabs>
...
export default {
  ...
  data() {
    return {
      tabModel: '',
      currentScoreGroupId: null,
      currentScoreGroupIndex: 0,
      headers: [
        {
          text: '',
          value: 'delete',
  ...
  created() {
    if (process.browser) {
      this.GetScoresByScore()
    }
  },
  methods: {
    async GetScoresByScore() {
      this.loading = true
      this.Scores = await this.$axios.$get('/scores/students/score')
    ...
    async RemoveStudent(student) {
      await this.$axios
        ...
        .then(() => {
          let index = this.Scores[this.currentScoreGroupIndex].scores
            .map(function(e) {
              return e.name
            })
            .indexOf(student.name)
          this.Scores[this.currentScoreGroupIndex].scores.splice(index, 1)
        })
    },
    SetScoreGroup(id, index) {
      this.currentScoreGroupId = id
      this.currentScoreGroupIndex = index
    }
  }
}
</script>
