    ...
    <v-flex xs12>
      <v-card>
        <v-toolbar flat color="yellow darken-1">
          <v-toolbar-title>
            Year Averages Above 4.5
          </v-toolbar-title>
          <v-spacer />
          <v-layout>
            <v-flex xs10 class="text-xs-right pt-3 subheading">
              Year:
            </v-flex>
...
export default {
  ...
  methods: {
    async GetScores() {
      if (this.selectedYear != null) {
        ...
        this.Scores = await this.$axios.$get(
          `/scores/students/fourpointfive/${this.selectedYear}/false`
        )
        setTimeout(() => {
          this.loading = false
    ...
    async DownloadCSV() {
      if (this.selectedYear != null) {
        await this.$axios
          .$get(`/scores/students/fourpointfive/${this.selectedYear}/true`)
          .then(response => {
